...
Function Main() {
    ...
    for ($i = $startRow; $i -le $endRow; $i++)
    {
        ...
        if ($currentCommonFondValue -or $currentGroupValue) {
            Write-Host "Для группы $groupNumber на листе $GroupSheetName для $incomingName в месяце $monthName уже есть значение: $currentCommonFondValue и $currentGroupValue, взнос будет пропущен!" -ForegroundColor Red
            continue
        }
